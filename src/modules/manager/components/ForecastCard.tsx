      ...
      <div className="mb-3 flex items-baseline gap-2">
        ...
        {data && (
          <span className={`ml-auto inline-flex items-center gap-0.5 rounded-full px-2 py-0.5 text-xs font-semibold ${
            data.tendencia_pct >= 0 ? 'bg-[var(--mint-glow)] text-[var(--mint)]' : 'bg-[oklch(30%_.12_25_/_0.12)] text-[var(--coral)]'
          }`}>
        ...
        <>
          <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
            <div>
              ...
              <div className="font-display text-2xl font-bold text-[var(--mint)] tabular-nums">{eur(data.mes_actual_proy)}</div>
            ...
            <div>
              ...
              <div className="font-display text-2xl font-bold text-[oklch(44%_.12_224)] tabular-nums dark:text-[oklch(76%_.12_224)]">{eur(data.forecast_next)}</div>
            ...
            <div className="col-span-2 md:col-span-1">
              ...
              <div className={`font-display text-2xl font-bold tabular-nums ${data.tendencia_pct >= 0 ? 'text-[var(--mint)]' : 'text-[var(--coral)]'}`}>
                ...
                <BarChart data={data.meses_serie} margin={{ top: 18, right: 8, left: -10, bottom: 0 }}>
                  ...
                  <Bar dataKey="ventas" radius={[4, 4, 0, 0]}>
                    ...
                    {data.meses_serie.map((d, i) => (
                      <Cell key={i} fill={d.es_proy ? '#3b82f6' : '#34d399'} fillOpacity={d.es_proy ? 0.6 : 1} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
              <div className="mt-1 flex items-center justify-end gap-3 text-[10px] text-[var(--color-ink-3)]">
                <span className="flex items-center gap-1"><span className="h-2 w-2 bg-[var(--mint)]"></span>Real</span>
