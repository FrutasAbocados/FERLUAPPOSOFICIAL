        ...
        <div className="space-y-5 px-5 py-4">
          ...
          <section className="rounded-lg border border-[var(--color-border)] p-4">
            ...
            {empleado.exceso_nuevo > 0 && (
              <p className="ao-tone-danger mt-3 rounded-md px-3 py-2 text-xs">
          ...
          <section className="rounded-lg border border-[var(--color-border)] p-4">
                    ...
                    <div className="grid grid-cols-1 gap-2 md:grid-cols-[1fr_80px_100px_100px_auto] md:items-end">
                      ...
                      <div className="grid grid-cols-[1fr_1fr_auto_auto] items-end gap-2 md:contents">
                        ...
                        <Button size="sm" variant="ghost" onClick={() => removeLinea(i)} title="Eliminar">
                          <Trash2 className="ao-text-danger h-4 w-4" />
                        </Button>
                      </div>
                    </div>
                  </li>
            ...
            <div className="mt-3 flex items-center justify-between border-t border-[var(--color-border)] pt-3">
              ...
              <div className="flex items-center gap-3">
                <span className="ao-text-success text-base font-bold tabular-nums">{eur(totalForm)}</span>
                  ...
                  return (
                    <li key={m.mes}>
                      ...
                      >
                        ...
                        {m.exceso_nuevo > 0 ? (
                          <span className="ao-tone-danger rounded-full px-2 py-0.5 text-[10px] font-semibold">+{eur(m.exceso_nuevo)} exceso</span>
                        ) : (
                          <span className="ao-tone-success rounded-full px-2 py-0.5 text-[10px] font-semibold">ok</span>
                        )}
                      </button>
                      {abierto && (
...
function Kpi({ label, value, tone = 'neutral' }: { label: string; value: string; tone?: 'neutral' | 'green' | 'red' | 'amber' }) {
  const color =
    tone === 'green' ? 'ao-text-success' :
    tone === 'red' ? 'ao-text-danger' :
    tone === 'amber' ? 'ao-text-warning' :
    'text-[var(--color-ink)]'
  return (
