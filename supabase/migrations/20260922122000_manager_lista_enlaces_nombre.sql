...
AS $function$
  ...
  productos as (
    select
      agg.*,
      coalesce(nullif(agg.product_id_original, '0'), ph.holded_product_id) as pid
    from agg
    left join lateral (
      select m.holded_product_id
      from (
        select p.holded_product_id,
          case when lower(trim(agg.nombre)) = lower(p.holded_product_name) then 0 else 1 end as prioridad,
          p.updated_at, p.producto_normalizado as clave
        from public.pedidos_wa_productos_holded p
        where nullif(agg.product_id_original, '0') is null
          and p.holded_product_id <> '0'
          and (lower(trim(agg.nombre)) = lower(p.holded_product_name)
            or lower(trim(agg.nombre)) = p.producto_normalizado)
        union all
        select a.holded_product_id, 2, a.updated_at, a.nombre_norm
        from public.manager_coste_nombre_auto a
        where nullif(agg.product_id_original, '0') is null
          and a.holded_product_id <> '0'
          and a.nombre_norm = lower(trim(agg.nombre))
      ) m
