-- RPC pedidos_wa_resolver_completo:
--   1) Añade 3er nivel de fallback de precio: 'ultima_venta_global' = última
--      venta histórica del producto a cualquier cliente, sin límite de fecha.
--      Evita que líneas se vayan a 0€ a Holded cuando no hay histórico_cliente
--      ni tarifa_base 60d. Acepta margen de error porque Luis revisa borrador.
--   2) Compacta el formato de trazabilidad: 'LYYMMDD · NUMFACTURA' en lugar
--      del nombre largo del proveedor. Cuando el módulo Compras Factura esté
--      LIVE y suban facturas proveedor, esa trazabilidad real (con código
--      proveedor) sobreescribirá el fallback Holded.
...
as $$
  ...
  -- Nivel 3 fallback: última venta del producto a cualquier cliente, sin
  -- límite de fecha. Solo se usa si no hay histórico cliente ni tarifa base.
  ...
  select
    ...
    case
      -- Prioridad 2 (futura): trazabilidad parser factura proveedor.
      -- Cuando esté LIVE el módulo Compras Factura, esta fuente debería
      -- traer un código del proveedor real, así que va por encima de Holded.
      when tcw.proveedor_nombre is not null then
        'L' || to_char(tcw.fecha_compra, 'YYMMDD')
            || coalesce(' · ' || tcw.num_factura, '')
      -- Prioridad 1 actual: COMPRA Holded por product_id.
  ...
  left join ultima_venta_global uvg on uvg.prod_key = l.prod_lower
