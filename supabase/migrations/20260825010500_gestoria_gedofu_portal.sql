-- Portal de gestoría Gedofu.
--
-- Frontera de seguridad:
-- - el rol no recibe SELECT sobre tablas operativas ni acceso a Storage;
-- - las dos RPC SECURITY DEFINER solo devuelven columnas contables sanitizadas;
-- - nunca exponen raw, IDs de Holded, rutas, PDFs, fotos ni identificadores internos.

alter type public.app_role add value if not exists 'gestor_gedofu';

create or replace function public.es_gestor_gedofu()
returns boolean
language sql
stable
security invoker
...
revoke all on function public.es_gestor_gedofu() from public;
...
grant execute on function public.es_gestor_gedofu() to authenticated;
