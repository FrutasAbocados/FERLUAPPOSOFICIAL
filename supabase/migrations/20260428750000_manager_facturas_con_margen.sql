-- ============================================================================
-- Manager — facturas_lista incluye margen agregado de la factura
-- ============================================================================
-- Añade cogs y margen (sum de líneas con coste) por factura. Sólo tiene sentido
-- en VENTAS; en COMPRAS los valores serán 0.
-- ============================================================================

drop function if exists public.manager_facturas_lista(date, date, text, text, text, int);
...
returns table(
  ...
  select
    ...
    coalesce(m.margen, 0)      as margen,
    case when coalesce(m.ventas_lineas, 0) > 0
         then round((m.margen / m.ventas_lineas) * 100, 1)
