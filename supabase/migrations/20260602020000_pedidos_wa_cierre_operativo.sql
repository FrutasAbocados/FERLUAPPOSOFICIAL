-- Pedidos WA: cierre operativo diario para sustituir Excel manual.
-- Persiste montaje de ruta y añade sugerencias de faltas/compra por proveedor.

create table if not exists public.pedidos_wa_ruta_config (
  fecha date not null,
  repartidor text not null check (repartidor in ('TORRES','GERMAN','RAUL','ALEX')),
  salida text not null default 'PRIMERA' check (salida in ('PRIMERA','SEGUNDA')),
  vehiculo text,
  updated_at timestamptz not null default now(),
  primary key (fecha, repartidor, salida)
);

create table if not exists public.pedidos_wa_ruta_extras (
...
create index if not exists pedidos_wa_ruta_extras_fecha_idx
  on public.pedidos_wa_ruta_extras (fecha, repartidor, salida, orden);

create table if not exists public.pedidos_wa_producto_proveedor (
  producto_key text primary key,
  proveedor text not null check (proveedor in ('alcalde','abasthosur','mercado','otro')),
  updated_at timestamptz not null default now()
);

drop trigger if exists pedidos_wa_ruta_config_touch on public.pedidos_wa_ruta_config;
create trigger pedidos_wa_ruta_config_touch before update on public.pedidos_wa_ruta_config
for each row execute function public.touch_updated_at();

drop trigger if exists pedidos_wa_ruta_extras_touch on public.pedidos_wa_ruta_extras;
create trigger pedidos_wa_ruta_extras_touch before update on public.pedidos_wa_ruta_extras
for each row execute function public.touch_updated_at();

drop trigger if exists pedidos_wa_producto_proveedor_touch on public.pedidos_wa_producto_proveedor;
create trigger pedidos_wa_producto_proveedor_touch before update on public.pedidos_wa_producto_proveedor
for each row execute function public.touch_updated_at();

alter table public.pedidos_wa_ruta_config enable row level security;
alter table public.pedidos_wa_ruta_extras enable row level security;
alter table public.pedidos_wa_producto_proveedor enable row level security;

create policy "pedidos_wa_ruta_config: admin rw" on public.pedidos_wa_ruta_config
for all using (is_admin()) with check (is_admin());
create policy "pedidos_wa_ruta_config: operaciones read" on public.pedidos_wa_ruta_config
for select using (puede_operar_pedidos_wa() or es_responsable());

create policy "pedidos_wa_ruta_extras: admin rw" on public.pedidos_wa_ruta_extras
for all using (is_admin()) with check (is_admin());
create policy "pedidos_wa_ruta_extras: operaciones read" on public.pedidos_wa_ruta_extras
for select using (puede_operar_pedidos_wa() or es_responsable());

create policy "pedidos_wa_producto_proveedor: admin rw" on public.pedidos_wa_producto_proveedor
for all using (is_admin()) with check (is_admin());
create policy "pedidos_wa_producto_proveedor: operaciones read" on public.pedidos_wa_producto_proveedor
for select using (puede_operar_pedidos_wa() or es_responsable());

grant select, insert, update, delete on public.pedidos_wa_ruta_config to authenticated;
grant select, insert, update, delete on public.pedidos_wa_ruta_extras to authenticated;
grant select, insert, update, delete on public.pedidos_wa_producto_proveedor to authenticated;
